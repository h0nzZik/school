 ...
 *                  KEYBOARD - Example
 *
 * Description : Basic functions used for matrix keyboard, second method of key reading
 *
 *               
 *
 * Author: Zbynek Bures
...
#define KEY_WAS_PRESSED     0x100
#define KEY_WAS_HOLD        0x200
#define KEY_WAS_CHANGED     0x400
#define KEY_WAS_RELEASED    0x800
...
int main(void)
{
	uint16_t tmp_key = KEY_WAS_RELEASED;												/* temporary */
	...
	while (1) {
		if (Key_scan() || ((tmp_key & 0xff00) != KEY_WAS_RELEASED)) {
			tmp_key = Key_read();														/* If we found new key, read it */
			printf ("Button number %d was ",tmp_key & 0xff);
			switch (tmp_key & 0xff00)
			{
				case 0x100:	printf("pressed.\n"); break;
				case 0x200:	printf("hold.\n"); break;
				case 0x400:	printf("changed.\n"); break;
				case 0x800:	printf("released.\n"); break;
			}
		}
	}	
	return(0);
...
/*!
 * \brief Function reads keyboard and return information about pressed and released key.
 *	Function waits untill button is stable, than function returns its value and information that key is pressed.
 *  When key is released, it inform againg
 *
 * \return key_read => low byte is Keycode of pushed button, high byte is information about pressing
 *
 */
uint16_t Key_read(void) {
static uint16_t key = 0;
    ...
    key_tmp = Key_scan();
    /* Did we read a key ? */
    if ((key & 0xff00) != KEY_WAS_RELEASED) {
        /* <Y> Do we have the same key ? */
        if ((key & 0xff) == key_tmp)
            key = (key & 0xff) | KEY_WAS_HOLD;           /* <Y> Change a flag */
        /* <N> Did he release a key ? */
        else if (key_tmp & 0xff)
            key = key_tmp | KEY_WAS_CHANGED;            /* <N> He changed a key */
        else
            key = (key & 0xff) | KEY_WAS_RELEASED;                     /* <N> He released a key */
    }
    else {
        /* This the first time we read a key */
        do {
